...
class DetPredLayer(nn.Module):
    def __init__(self,
        ...
        # ------------------ Model parameters -------------------
        ## Pred layers
        self.cls_pred = nn.Conv2d(cls_head_dim, num_classes, kernel_size=3, padding=1)
        self.reg_pred = nn.Conv2d(reg_head_dim, 4, kernel_size=3, padding=1)
    ...
    def forward(self, cls_feats, reg_feats, mask=None):
        all_masks = []
        all_anchors = []
        all_cls_preds = []
        all_reg_preds = []
        all_box_preds = []
        for lvl, (cls_feat, reg_feat) in enumerate(zip(cls_feats, reg_feats)):
            # ------------------- Generate anchor box -------------------
            B, _, H, W = cls_feat.size()
            fmp_size = [H, W]
            anchors = self.get_anchors(lvl, fmp_size)   # [M, 4]
            anchors = anchors.to(cls_feat.device)
            ...
            cls_pred = cls_pred.permute(0, 2, 3, 1).contiguous().view(B, -1, self.num_classes)
            reg_pred = reg_pred.permute(0, 2, 3, 1).contiguous().view(B, -1, 4)
            box_pred = self.decode_boxes(reg_pred, anchors, self.stride[lvl])
            ## Adjust mask
            if mask is not None:
                # [B, H, W]
                mask_i = torch.nn.functional.interpolate(mask[None].float(), size=[H, W]).bool()[0]
